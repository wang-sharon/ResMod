...
# Load packages
library(tidyverse)
library(simcf) # simulate counterfactuals
...
clean_data$vic <- factor(clean_data$vic, levels = c("CSA only", "ASA only", "CSA + ASA"))
...
# Set up linear regression of model
Model1_result <- lm(Model1, data = clean_data)
pe_Model1 <- Model1_result$coefficients
vc_Model1 <- vcov(Model1_result)

# Simulate quantites of interest
sims <- 100
simbetas <- mvrnorm(sims, pe_Model1, vc_Model1)

# ... not sure where to go next - to come back to!!
