...
# reverse DERS
raw_data[, DERS] <- lapply(raw_data[, DERS], as.numeric)

# one item first
new_dataset <- 6 - new_dataset[,DERS]
new_dataset$ERS <- rowSums(new_dataset[,DERS], na.rm = T)
new_dataset$ID <- raw_data$ID
new_dataset$DERS_TotalScore <- raw_data$DERS_TotalScore

# calculate what the expected ERS score is based off of DERS score
new_dataset$Expected_ERS <- (6 * 36) - new_dataset$DERS_TotalScore

# Identify discrepancies where ERS does not match Expected_ERS
new_dataset[new_dataset$ERS != new_dataset$Expected_ERS, 
            c("ID", "DERS_TotalScore", "ERS", "Expected_ERS")]

discrepant_IDs <- new_dataset$ID[new_dataset$ERS != new_dataset$Expected_ERS]

discrepant_cases <- new_dataset[new_dataset$ID %in% discrepant_IDs, ]

#
discrepant_cases[,DERS] <- lapply(discrepant_cases[,DERS], as.numeric)
...
raw_data$original_ERS <- -raw_data$DERS_TotalScore # this is incorrect
...
final_outcomes <- c("PTSD", "ERS", "SMQ_tot", "original_ERS")
