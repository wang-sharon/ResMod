...
# check if altered item affects results
raw_data$PCL_adjusted <- raw_data$PTSD - raw_data$ptsd12
raw_data$PCL_adjusted <- replace(raw_data$PCL_adjusted, raw_data$PCL_adjusted < 0, 0)
...
VOI_data <- raw_data[c("ID", "PTSD", "PCL_adjusted", "csayn", AdolSA, AdultSA, "ERS",
                       "SMQ_tot", demo)]
...
final_outcomes <- c("PCL_adjusted", "PTSD", "ERS", "SMQ_tot")
...
correlation_variables <- c("PCL_adjusted", "ERS", "SMQ_tot")
...
pcl_items <- raw_data[, c("ID", "ptsd01", "ptsd02", "ptsd03", "ptsd04", "ptsd05", "ptsd06",
                         "ptsd07", "ptsd08", "ptsd09", "ptsd10", "ptsd11", "ptsd12", "ptsd13", 
                         "ptsd14", "ptsd15", "ptsd16", "ptsd17", "ptsd18", "ptsd19","ptsd20")]
pcl_items <- pcl_items[pcl_items$ID %in% clean_data$ID, ]
pcl_items <- pcl_items[,-1]

alpha_full <- psych::alpha(pcl_items)
alpha_noFF <- psych::alpha(pcl_items[,-12])

alpha_full$total$raw_alpha 
alpha_noFF$total$raw_alpha

# reviewer suggested that demographic variables might influence ptss severity
correlation_demographics <- c("PTSD", "age", "race", "income")

corr_demo_results <- data.frame(Variable1 = character(), 
                                Variable2 = character(), 
                                Correlation = numeric(), P_Value = numeric(), 
                                tringsAsFactors = FALSE)

for (var1 in 1:(length(correlation_demographics) - 1)) {
  for (var2 in (var1 + 1):length(correlation_demographics)) {
    # Calculate correlation and p-value
      demo_result <- cor.test(clean_data[[correlation_demographics[var1]]], 
                                        clean_data[[correlation_demographics[var2]]])
      
        # Store results in the data frame
        corr_demo_results <- rbind(corr_demo_results, data.frame(
          Variable1 = correlation_demographics[var1],
          Variable2 = correlation_demographics[var2],
          Correlation = demo_result$estimate,
          P_Value = demo_result$p.value
          ))
        }
}

# reviewer wondered whether included vs. excluded people differed
# split by included vs. excluded
VOI_data<- VOI_data %>%
  mutate(included = ifelse(ID %in% clean_data$ID, 1, 0))

# Numeric variables comparison
numeric_vars <- c("age", "PTSD", "csayn", "ASA")

# Loop through numeric variables and run t-tests
for (var in numeric_vars) {
  t_test <- t.test(as.formula(paste(var, "~ included")), data = VOI_data)
  cat("Variable:", var, "\n")
  print(t_test)
  cat("\n--------------------\n")
}

# Categorical variables comparison (e.g., race)
categorical_vars <- c("race")

for (var in categorical_vars) {
  tab <- table(VOI_data[[var]], VOI_data$included)
  cat("Variable:", var, "\n")
  print(tab)
