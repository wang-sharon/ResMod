...
VOI_data$vic <- ifelse(VOI_data$CSA_only == 1, "CSA_only",
                       ifelse(VOI_data$ASA_only == 1, "ASA_only",
                              ifelse(VOI_data$CSA_ASA == 1, "CSA_ASA",
                                     ifelse(VOI_data$CSA_only == 0 &
                                              VOI_data$ASA_only == 0 &
                                              VOI_data$CSA_ASA == 0, "no_vic", NA))))

vic_variables <- c("csayn", "ASA")

# Final data ---------------------------------------------------------

final_outcomes <- c("PTSD", "PosDERS", "SMQ_tot", "CSI_Total")

clean_data <- VOI_data[c("ID", vic_variables, final_outcomes, "vic")]
...
correlation_variables <- c("PTSD", "PosDERS", "SMQ_tot", "CSI_Total")
